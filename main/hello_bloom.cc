...
class Bloom final : public Scene
{
 ...
 private:
  ...
  Shader* shader_blur_ = nullptr;
  Shader* shader_bloom_ = nullptr;
...
void Bloom::Begin()
{
  ...
  //Build shaders
  shader_ = new Shader("data/shaders/hello_bloom/bloom.vert", "data/shaders/hello_bloom/bloom.frag");
  light_ = new Shader("data/shaders/hello_bloom/bloom.vert", "data/shaders/hello_bloom/light.frag");
  shader_blur_ = new Shader("data/shaders/hello_bloom/blur.vert", "data/shaders/hello_bloom/blur.frag");
  shader_bloom_ =  new Shader("data/shaders/hello_bloom/bloom_final.vert", "data/shaders/hello_bloom/bloom_final.frag");

  //load textures
  ground_texture_ = TextureFromFile("marble.jpg", "data/textures");
  ...
  shader_bloom_->use();
  shader_bloom_->SetInt("scene", 0);
  shader_bloom_->SetInt("bloomBlur", 1);
}
...
void Bloom::Update(const float dt)
{
  ...
  light_->use();
  light_->SetMat4("projection", projection);
  light_->SetMat4("view", view);
  ...
  for (unsigned int i = 0; i < light_positions_.size(); i++)
  {
    ...
    light_->SetMat4("model", model);
    light_->SetVec3("lightColor", light_colors_[i]);
    renderCube();
  }
  glBindFramebuffer(GL_FRAMEBUFFER, 0);
  ...
  shader_bloom_->SetInt("bloom", bloom_state_);
  shader_bloom_->SetFloat("exposure", exposure_);
  renderQuad();
