...
void HDR::Begin()
{
  ...
  //Main program(s)
  light_ = new Shader("data/shaders/hdr/light.vert", "data/shaders/hdr/light.frag");
  hdr_ = new Shader("data/shaders/hdr/hdr.vert", "data/shaders/hdr/hdr.frag");
  ...
  light_->use();
  light_->SetInt("diffuseTexture", 0);

  hdr_->use();
  hdr_->SetInt("hdrBuffer", 0);
}
...
void HDR::Update(const float dt)
{
  ...
  {
    light_->SetVec3("lights[" + std::to_string(i) + "].Position", light_positions_[i]);
    light_->SetVec3("lights[" + std::to_string(i) + "].Color", light_colors_[i]);
  }
  light_->SetVec3("viewPos", camera_->camera_position_);
  // render tunnel
  auto model = glm::mat4(1.0f);
  model = glm::translate(model, glm::vec3(0.0f, 0.0f, 25.0));
  model = glm::scale(model, glm::vec3(2.5f, 2.5f, 27.5f));
  light_->SetMat4("model", model);
  light_->SetInt("inverse_normals", true);
  renderCube();
  glBindFramebuffer(GL_FRAMEBUFFER, 0);
  ...
  glActiveTexture(GL_TEXTURE0);
  glBindTexture(GL_TEXTURE_2D, color_buffer_);
  hdr_->SetInt("hdr", hdr_bool_);
  hdr_->SetFloat("exposure", exposure_);
  renderQuad();

  glBindVertexArray(0);
}

void HDR::OnEvent(const SDL_Event &event, const float dt)
{
  // Get keyboard state
...
void HDR::DrawImGui()
{
  ...
  ImGui::Checkbox("Enable HDR", &hdr_bool_);
